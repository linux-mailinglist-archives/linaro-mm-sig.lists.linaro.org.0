Return-Path: <linaro-mm-sig-bounces@lists.linaro.org>
X-Original-To: lists+linaro-mm-sig@lfdr.de
Delivered-To: lists+linaro-mm-sig@lfdr.de
Received: from lists.linaro.org (lists.linaro.org [107.22.173.205])
	by mail.lfdr.de (Postfix) with ESMTPS id 78797390916
	for <lists+linaro-mm-sig@lfdr.de>; Tue, 25 May 2021 20:37:55 +0200 (CEST)
Received: from lists.linaro.org (localhost [127.0.0.1])
	by lists.linaro.org (Postfix) with ESMTP id 66C5D61170
	for <lists+linaro-mm-sig@lfdr.de>; Tue, 25 May 2021 18:37:53 +0000 (UTC)
Received: by lists.linaro.org (Postfix, from userid 109)
	id 570D46140F; Tue, 25 May 2021 18:37:51 +0000 (UTC)
Received: from lists.linaro.org (localhost [127.0.0.1])
	by lists.linaro.org (Postfix) with ESMTP id 0121160B99;
	Tue, 25 May 2021 18:37:48 +0000 (UTC)
X-Original-To: linaro-mm-sig@lists.linaro.org
Delivered-To: linaro-mm-sig@lists.linaro.org
Received: from lists.linaro.org (localhost [127.0.0.1])
 by lists.linaro.org (Postfix) with ESMTP id 17D9760B8B
 for <linaro-mm-sig@lists.linaro.org>; Tue, 25 May 2021 18:37:46 +0000 (UTC)
Received: by lists.linaro.org (Postfix, from userid 109)
 id 0D35E60B99; Tue, 25 May 2021 18:37:46 +0000 (UTC)
Received: from mail-qv1-f74.google.com (mail-qv1-f74.google.com
 [209.85.219.74])
 by lists.linaro.org (Postfix) with ESMTPS id EDBB460B8B
 for <linaro-mm-sig@lists.linaro.org>; Tue, 25 May 2021 18:37:43 +0000 (UTC)
Received: by mail-qv1-f74.google.com with SMTP id
 w14-20020a056214012eb02901f3a4388530so23024655qvs.17
 for <linaro-mm-sig@lists.linaro.org>; Tue, 25 May 2021 11:37:43 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=google.com; s=20161025;
 h=date:message-id:mime-version:subject:from:to:cc
 :content-transfer-encoding;
 bh=jncM/ExWNLmYNSUtVvhJ+5KKUC85VKAKmLpNEbRZsSk=;
 b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=1e100.net; s=20161025;
 h=x-gm-message-state:date:message-id:mime-version:subject:from:to:cc
 :content-transfer-encoding;
 bh=jncM/ExWNLmYNSUtVvhJ+5KKUC85VKAKmLpNEbRZsSk=;
 b=
X-Gm-Message-State: AOAM530F04vK48u9YqIJ9FjF1/LM7T4B+TU4LF9gi7WcRhVLsvsD2ZuI
 jguMk/gh5bG6dPq9x4wy9l8M7H0y0cY=
X-Google-Smtp-Source: ABdhPJz8Hork/L4eFR9k5l3K9ZysAlbbOfNymh6eNt4vWiQw4Bf5k3kHm5YTo6FY6LB0lbu0WeZ7t06Zpwk=
X-Received: from hridya.mtv.corp.google.com
 ([2620:15c:211:200:b209:4c48:98d3:a871])
 (user=hridya job=sendgmr) by 2002:ad4:5907:: with SMTP id
 ez7mr37858323qvb.35.1621967863438; 
 Tue, 25 May 2021 11:37:43 -0700 (PDT)
Date: Tue, 25 May 2021 11:37:13 -0700
Message-Id: <20210525183720.1739480-1-hridya@google.com>
Mime-Version: 1.0
X-Mailer: git-send-email 2.31.1.818.g46aad6cb9e-goog
From: Hridya Valsaraju <hridya@google.com>
To: daniel@ffwll.ch, Sumit Semwal <sumit.semwal@linaro.org>, 
 "=?UTF-8?q?Christian=20K=C3=B6nig?=" <christian.koenig@amd.com>,
 Hridya Valsaraju <hridya@google.com>, 
 linux-kernel@vger.kernel.org, linux-media@vger.kernel.org, 
 dri-devel@lists.freedesktop.org, linaro-mm-sig@lists.linaro.org
X-Virus-Scanned: ClamAV using ClamSMTP
Subject: [Linaro-mm-sig] [PATCH v4] dmabuf: Add the capability to expose
 DMA-BUF stats in sysfs
X-BeenThere: linaro-mm-sig@lists.linaro.org
X-Mailman-Version: 2.1.29
Precedence: list
List-Id: "Unified memory management interest group."
 <linaro-mm-sig.lists.linaro.org>
List-Unsubscribe: <https://lists.linaro.org/mailman/options/linaro-mm-sig>,
 <mailto:linaro-mm-sig-request@lists.linaro.org?subject=unsubscribe>
List-Archive: <http://lists.linaro.org/pipermail/linaro-mm-sig/>
List-Post: <mailto:linaro-mm-sig@lists.linaro.org>
List-Help: <mailto:linaro-mm-sig-request@lists.linaro.org?subject=help>
List-Subscribe: <https://lists.linaro.org/mailman/listinfo/linaro-mm-sig>,
 <mailto:linaro-mm-sig-request@lists.linaro.org?subject=subscribe>
Cc: john.stultz@linaro.org, gregkh@linuxfoundation.org, kernel-team@android.com,
 surenb@google.com, kernel test robot <lkp@intel.com>
Content-Type: text/plain; charset="utf-8"
Content-Transfer-Encoding: base64
Errors-To: linaro-mm-sig-bounces@lists.linaro.org
Sender: "Linaro-mm-sig" <linaro-mm-sig-bounces@lists.linaro.org>
X-Virus-Scanned: ClamAV using ClamSMTP


